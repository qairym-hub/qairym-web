...
class AuthController {
  ...
  async login(username, password, callback) {
    try {
      const response = await axios.post(`https://qairhub.herokuapp.com/login`, {
        username: username,
        password: password,
      });
      ...
      if (axios.isAxiosError(error)) return callback(null, error);
  ...
  async create(user, onSuccess, onError) {
    try {
      const response = await axios.post(
        `https://qairhub.herokuapp.com/api/user/save`,
        user
      );

      return onSuccess(response.data);
    } catch (error) {
      if (axios.isAxiosError(error)) return onError(null, error);
  ...
  async loadByToken(token) {
    ...
    const response = await axios.get(
      `https://qairhub.herokuapp.com/api/user/profile`,
      header
    );
